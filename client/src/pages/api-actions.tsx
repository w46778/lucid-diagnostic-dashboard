    ...
    <DashboardLayout title="Decoded API Actions" subtitle="Community-decoded Lucid Motors API methods from nshp/python-lucidmotors">
      ...
      <div className="mb-4 flex flex-wrap items-center gap-3">
        ...
        <button
          ...
          data-testid="button-tested-only"
          className={cn(
            'flex items-center gap-2 rounded-md border px-3 py-2 text-sm transition-colors',
            showTestedOnly ? 'border-green-500/30 bg-green-500/10 text-green-400' : 'border-border hover:bg-accent'
          )}
        >
        ...
        <Card>
          <CardHeader>
            <CardTitle className="flex items-center gap-2 text-sm">
              ...
              API Method Repository
              <span className="text-xs text-muted-foreground">({actions.length} shown)</span>
            </CardTitle>
          </CardHeader>
          <CardContent>
              ...
              <table className="w-full text-sm" data-testid="table-actions">
                ...
                <tbody>
                  {actions.map((action: any) => (
                    <tr key={action.name} className="border-b border-border/50 last:border-0 hover:bg-accent/30" data-testid={`action-${action.name}`}>
                      <td className="py-3 pr-4">
                        <p className="font-mono text-sm font-semibold text-primary">{action.name}</p>
                      </td>
                      <td className="py-3 pr-4">
                        <span className="rounded-full bg-muted px-2 py-0.5 text-xs text-muted-foreground">
                          {categoryLabels[action.category as ActionCategory] || action.category}
                        </span>
                      </td>
                      ...
                      <td className="py-3 pr-4">
                        <code className="rounded bg-muted px-1.5 py-0.5 text-xs text-muted-foreground">{action.methodSignature}</code>
                      </td>
                      <td className="py-3 pr-4">
                        {action.testedInActions ? (
                          <span className="flex items-center gap-1 text-xs text-green-400" data-testid={`tested-${action.name}`}>
      ...
      <Card className="mt-6">
        ...
        <CardContent>
          <p className="text-sm text-muted-foreground">
            The community stress-test script that runs through every decoded API action. Located at{' '}
            <a href={data?.testScriptUrl} target="_blank" rel="noopener noreferrer" className="text-primary hover:underline" data-testid="link-test-script">
              examples/test_all_actions.py <ExternalLink className="inline h-3 w-3" />
            </a>
            . Tests {data?.testedInScript} actions including wake, charge port, defrost, doors, frunk, trunk, horn, lights, and vehicle refresh.
          </p>
        </CardContent>
      </Card>
    </DashboardLayout>
...
function CategoryButton({ label, active, onClick, count }: { label: string; active: boolean; onClick: () => void; count: number }) {
  return (
    <button
      onClick={onClick}
      data-testid={`filter-action-${label.toLowerCase().replace(/\s+/g, '-')}`}
      className={cn(
