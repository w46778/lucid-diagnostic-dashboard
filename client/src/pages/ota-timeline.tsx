...
import type { OtaTimelineResponse } from '@/types/api';
...
export default function OtaTimeline() {
  ...
  const { data, isLoading } = useQuery<OtaTimelineResponse>({
    queryKey: ['/api/ota-timeline'],
  });

  const updates = (data?.updates ?? []).filter((update) => filter === 'all' || update.category === filter).slice().reverse();
        ...
        <div className="relative" data-testid="timeline">
          ...
          <div className="space-y-4">
            {updates.map((update, index) => {
              const config = categoryConfig[update.category] || categoryConfig.bugfix;
