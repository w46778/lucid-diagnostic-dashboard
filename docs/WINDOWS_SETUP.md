...
- builds a logical ECU-address inventory
...
4. Confirm `Vehicle Transmit = DISABLED`.
5. Confirm the expected Windows Ethernet adapter appears.
6. Confirm TShark capture interfaces are listed.
7. Save a readiness session.
...
6. Observe packet count, DoIP frame count, ECU logical addresses, and any already-present UDS traffic.
7. Stop capture.
8. Save the capture summary.
